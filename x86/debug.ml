...
let print_asmprog prog =
  ...
  let rec print_exp = function
    ...
    | Asm.Ld(x, V(y), z) -> printf "\tLD (%s,%s,%d)\n" x y z
    | Asm.Ld(x, C(i), z) -> printf "\tLD %d(%s,%d)\n" i x z
    | Asm.St(x, y, V(z), w) -> printf "\tST %s, (%s,%s,%d)\n" x y z w
    | Asm.St(x, y, C(i), w) -> printf "\tST %s, %d(%s,%d)\n" x i y w
    ...
    | Asm.LdDF(x, V(y), z) -> printf "\tLDDF (%s,%s,%d)\n" x y z
    | Asm.LdDF(x, C(i), z) -> printf "\tLDDF %d(%s,%d)\n" i x z
    | Asm.StDF(x, y, V(z), w) -> printf "\tSTDF %s, (%s,%s,%d)\n" x y z w
    | Asm.StDF(x, y, C(i), w) -> printf "\tSTDF %s, %d(%s,%d)\n" x i y w
