...
let rec fv = function (* 式に出現する（自由な）変数 (caml2html: knormal_fv) *)
  | Unit | Int(_) | Float(_) | ExtVar(_) | ExtArray(_) -> S.empty
  | Neg(x) | FNeg(x) -> S.singleton x
  | Mul(x, y) | Div(x, y) | Add(x, y) | Sub(x, y) | FAdd(x, y) | FSub(x, y) | FMul(x, y) | FDiv(x, y) | Get(x, y) -> S.of_list [x; y]
...
let dummy_syntax x = (* K正規化の過程で生成される、元々存在しなかったトークンを生成 *)
  (x,("_",Lexing.dummy_pos,Lexing.dummy_pos))
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | (Syntax.Not(e),_) -> g env (dummy_syntax(Syntax.If(e, dummy_syntax(Syntax.Bool(false)), dummy_syntax(Syntax.Bool(true)))))
  | (Syntax.Neg(e),_) ->
      insert_let (g env e)
  ...
  | (Syntax.If((Syntax.Not(e1),_), e2, e3),_) ->
